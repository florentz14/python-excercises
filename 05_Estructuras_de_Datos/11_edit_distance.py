# Archivo: 44_04_edit_distance.py
# Descripción: Distancia de edición (Levenshtein)

print("=== 4. Distancia de Edición (Levenshtein) ===\n")


def edit_distance(str1, str2):
    """
    Distancia de edición (Levenshtein): mínimo de operaciones
    (insertar, eliminar, sustituir) para transformar str1 en str2.
    Complejidad: O(m * n)
    """
    m, n = len(str1), len(str2)
    dp = [[0] * (n + 1) for _ in range(m + 1)]

    for i in range(m + 1):
        dp[i][0] = i
    for j in range(n + 1):
        dp[0][j] = j

    for i in range(1, m + 1):
        for j in range(1, n + 1):
            if str1[i - 1] == str2[j - 1]:
                dp[i][j] = dp[i - 1][j - 1]
            else:
                dp[i][j] = 1 + min(
                    dp[i - 1][j],
                    dp[i][j - 1],
                    dp[i - 1][j - 1]
                )

    return dp[m][n]


if __name__ == "__main__":
    palabra1 = "kitten"
    palabra2 = "sitting"

    print(f"Palabra 1: '{palabra1}'")
    print(f"Palabra 2: '{palabra2}'")
    distancia = edit_distance(palabra1, palabra2)
    print(f"Distancia de edición: {distancia}")
    print(f"Operaciones necesarias para convertir '{palabra1}' en '{palabra2}'")
